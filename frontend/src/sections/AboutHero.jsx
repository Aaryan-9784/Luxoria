  ...
  return (
    <section className="relative w-full h-[100vh] min-h-[820px] max-h-[1080px] flex items-center overflow-hidden pt-[110px] pb-[50px] bg-white">
      ...
      <div className="absolute top-0 right-0 w-full lg:w-[65%] h-full z-0 overflow-hidden pointer-events-none flex items-center justify-end">
        ...
        >
          <LuxuryImage
            src="https://images.unsplash.com/photo-1631269389578-8317e08929bc?q=80&w=2070&auto=format&fit=crop" 
      ...
      <div className="container-luxe relative z-20 grid lg:grid-cols-12 gap-8 items-center h-full">
        ...
        >
          ...
          <h1 className="text-[clamp(45px,5vw,75px)] leading-[1.1] font-bold text-[#0A0A0A] tracking-[-0.01em] mb-9 uppercase flex flex-col antialiased drop-shadow-[0_10px_30px_rgba(255,255,255,0.9)]">
            ...
            <span className="relative z-10 text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] via-[#E5C76B] to-[#C9A227] italic font-normal lowercase text-[clamp(40px,4.5vw,65px)] my-1 translate-x-5 font-serif drop-shadow-[0_2px_15px_rgba(212,175,55,0.15)] tracking-normal">extraordinary</span>
            <span className="relative z-10">Experiences</span>
          </h1>

          {/* Subheadline */}
          <p className="text-[16px] text-[#6B6B6B] leading-[2.2] mb-12 max-w-[500px] font-normal tracking-[0.02em] antialiased">
            Luxoria is redefining luxury mobility by providing access to the world's most prestigious vehicles and personalized concierge services.
          </p>
          ...
          <div className="flex flex-wrap items-center gap-10 mb-16">
            ...
            >
              ...
              <span className="relative z-10 font-bold tracking-[0.25em] uppercase text-[11px] group-hover:text-[#E5C76B] transition-colors duration-700 antialiased">Explore Collection</span>
            ...
            >
              <div className="w-[52px] h-[52px] rounded-full border border-black/10 flex items-center justify-center group-hover:border-[#C9A227]/40 group-hover:shadow-[0_0_30px_rgba(212,175,55,0.15)] transition-all duration-700 relative bg-white/40 backdrop-blur-xl">
                ...
                <Sparkles className="w-3.5 h-3.5 relative z-10 text-[#050505]/80 group-hover:text-[#C9A227] opacity-90 group-hover:opacity-100 transition-all duration-700" />
              </div>
              <span className="text-[11px] tracking-[0.25em] uppercase relative z-10 font-bold text-[#050505]/80 group-hover:text-[#C9A227] transition-colors duration-700 antialiased">Book Experience</span>
            </Link>
          </div>
          
          {/* Animated Statistics inside Hero */}
          <motion.div 
            className="grid grid-cols-2 md:grid-cols-4 gap-6 lg:gap-8 pt-8 border-t border-black/5"
          ...
          >
            {stats.map((stat, idx) => (
              <div key={idx} className="flex flex-col">
                <span className="text-2xl lg:text-3xl font-bold text-[#0A0A0A] tracking-tighter mb-1">{stat.value}</span>
                <span className="text-[10px] text-[#6B6B6B] uppercase tracking-[0.2em]">{stat.label}</span>
              </div>
            ))}
          </motion.div>

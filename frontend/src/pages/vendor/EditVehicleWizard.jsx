  ...
  useEffect(() => {
    const fetchVehicle = async () => {
      try {
        ...
        if (v.images) {
          setSelectedImages(v.images.map(img => img.url));
        }
      } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10">
      ...
      <div className="text-center">
        <h1 className="text-[32px] font-bold text-[#0F0F0F] tracking-tight mb-2" style={{ fontFamily: 'Georgia, "Times New Roman", serif' }}>Edit Vehicle</h1>
        <p className="text-[#666666] text-[13px] font-medium tracking-wide">Update your vehicle details and images.</p>
      </div>
            ...
            {currentStep === 2 && (
              ...
              >
                ...
                <div className="flex justify-between pt-8 border-t border-[#ECECEC]">
                  ...
                  >
                    {isSubmitting ? 'Processing...' : 'Update Vehicle Profile'} <ArrowRight className="w-4 h-4" />
                  </button>
                </div>
              </motion.div>
            )}
        ...
        <AnimatePresence>
          {currentStep === 3 && (
            <motion.div 
              ...
              className="space-y-8"
            >
              <h3 className="text-[13px] font-bold uppercase tracking-[0.15em] text-[#0F0F0F] border-b border-[#ECECEC] pb-4">Edit Vehicle Images (URLs)</h3>
              
              <div className="bg-[#16A34A]/10 text-[#16A34A] p-5 rounded-xl border border-[#16A34A]/20 flex items-center gap-3">
                <CheckCircle2 className="w-5 h-5 shrink-0" />
                <p className="text-[13px] font-bold">Vehicle profile updated successfully! Now manage up to 5 image URLs.</p>
              </div>

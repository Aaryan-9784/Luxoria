  ...
  return (
    <div ref={ref} className="relative w-full h-full">
      <button 
        ...
        className="flex items-center justify-between w-full cursor-pointer h-full outline-none"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-[-20px] md:left-0 mt-4 w-[240px] bg-white border border-border shadow-2xl rounded-2xl overflow-hidden z-[9999] p-2"
          >
...
export default function SmartSearch() {
  ...
  // Debounce search input changes and fetch
  useEffect(() => {
  ...
  // Debounce city input changes and fetch
  useEffect(() => {
    const timer = setTimeout(() => {
      if (filters.city !== localCity) {
        dispatch(setFilter({ city: localCity }));
        dispatch(fetchVehicles());
      }
    }, 500);
    return () => clearTimeout(timer);
  }, [localCity, dispatch, filters.city]);

  // Handle dropdown changes immediately and fetch
      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr_auto] gap-3 md:gap-0 items-center">
            ...
            {/* Brand */}
            <div className="flex items-center gap-3 px-4 py-3 md:py-2 bg-surface/60 md:bg-transparent rounded-xl md:rounded-none min-h-[56px] md:min-h-0">
            ...
            <div className="md:pl-3">
              <button
                type="submit"
                className="w-full md:w-auto btn btn-primary btn-lg rounded-xl md:rounded-full px-8 shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
              >

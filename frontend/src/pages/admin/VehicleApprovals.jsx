  ...
  return (
    <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-6">
          ...
          {['pending', 'approved', 'rejected'].map(status => (
            <button
              ...
              className={`btn btn-sm capitalize transition-all ${
                filter === status ? 'btn-primary' : 'btn-ghost text-secondary hover:text-primary'
              }`}
            >
              {status}
            </button>
      ...
      {loading && vehicles.length === 0 ? (
        ...
        <div className="matte-surface p-12 text-center flex flex-col items-center justify-center">
          <ShieldCheck className="w-12 h-12 text-muted mb-4" />
          <h3 className="text-h4 text-primary mb-2">No {filter} vehicles</h3>
          <p className="text-body-sm text-secondary">There are currently no vehicles in this queue.</p>
        </div>
      ) : (
            ...
            <motion.div variants={staggerItem} key={vehicle._id} className="card-lift flex flex-col relative group">
              ...
              <div className="relative h-48 bg-surface border-b border-border">
                ...
                <div className="absolute top-3 left-3">
                  <span className="badge bg-surface/90 backdrop-blur-sm text-primary border border-border">
                ...
                {vehicle.status === 'pending' && (
                  <div className="mt-auto grid grid-cols-2 gap-3 pt-4 border-t border-border">
                    <button 
                      ...
                      className="btn btn-outline text-error hover:border-error"
                    >

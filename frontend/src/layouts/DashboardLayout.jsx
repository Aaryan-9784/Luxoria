...
export default function DashboardLayout() {
  ...
  const renderSidebarContent = (collapsed = false, showToggle = false) => (
    <>
      <Link
        to="/"
        className={`flex items-center ${collapsed ? 'justify-center' : 'gap-4'} h-[80px] mb-4 shrink-0 px-4 group hover:opacity-80 transition-opacity`}
      ...
      >
        ...
        {!collapsed && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="flex flex-col justify-center items-start overflow-hidden whitespace-nowrap"
          >
            <span
              className="text-[22px] font-serif tracking-[0.12em] text-[#000000] leading-none font-bold"
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide flex flex-col">
        {NAV_GROUPS.map((group, idx) => (
          <div
            key={group.label}
            className={`flex flex-col gap-2.5 ${idx !== NAV_GROUPS.length - 1 ? 'mb-6' : 'mb-2'}`}
          >
            {!collapsed && (
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `relative flex items-center ${
                      collapsed ? 'justify-center w-[48px] mx-auto' : 'gap-3 mx-4 px-4'
                    } h-[48px] rounded-xl transition-all duration-300 ease-out group ${
                      isActive
                ...
                >
                  {({ isActive }) => (
                    <>
                      ...
                      <link.icon
                        className={`w-[20px] h-[20px] shrink-0 transition-transform duration-300 ${
                          isActive ? 'scale-110 text-[#C9A75D]' : 'group-hover:scale-110'
                        }`}
                      />
                      {!collapsed && (
                        <span className="font-semibold text-[13.5px] whitespace-nowrap">
                          {link.label}
                        </span>
                      )}
                    </>
      ...
      <div className="mt-auto shrink-0 px-4 py-3 flex flex-col gap-2 pt-4">
        <button
          ...
          className={`flex items-center justify-center gap-2 w-full h-[36px] rounded-lg text-[#5A1122] hover:bg-[#F3F4F6] transition-all duration-300 group ${
            collapsed ? 'px-0 mx-auto w-10' : 'px-3'
          }`}
  ...
  return (
    <div className="h-screen overflow-hidden bg-[#F8FAFC] flex">
        ...
        {showLogoutConfirm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#08152E]/40 backdrop-blur-sm p-4">
            ...
            >
              ...
              <h3 className="text-xl font-bold text-[#08152E] text-center mb-2">Sign Out?</h3>
              <p className="text-[13px] text-[#4B5563] text-center mb-8 leading-relaxed">
                You are about to sign out of your Luxoria account. Any unsaved changes may be lost.
              </p>
      ...
      {/* Desktop Sidebar */}
      <aside
        className={`hidden lg:flex flex-col h-screen sticky top-0 border-r border-[#E5E7EB] bg-white z-40 transition-all duration-300 ease-in-out relative ${
        ...
        {mobileMenuOpen && (
          <>
            ...
            >
              <button
                onClick={() => setMobileMenuOpen(false)}
                className="absolute top-6 right-4 p-1.5 rounded-full bg-[#F3F4F6] text-[#08152E] hover:bg-[#E5E7EB] transition-colors z-50"
              >
                <X className="w-4 h-4" />
              </button>
        ...
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-[#E5E7EB] px-6 lg:px-10 h-16 shrink-0 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              ...
              className="lg:hidden p-2 rounded-lg text-[#0F0F0F] hover:bg-[#F5F5F5]"
            >
            ...
            <div className="hidden lg:flex items-center gap-2.5">
              ...
              <Link
                to="/"
                className="group/home flex items-center justify-center w-8 h-8 rounded-xl bg-gradient-to-br from-[#C9A75D] to-[#A8843A] shadow-md hover:shadow-[0_0_12px_rgba(201,167,93,0.5)] hover:scale-105 active:scale-95 transition-all duration-200"
                title="Back to Home"
              >
              ...
              {/* Separator */}
              <svg width="6" height="14" viewBox="0 0 6 14" fill="none" className="opacity-25">
                <path d="M5 1L1 13" stroke="#0F0F0F" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
              ...
              {/* Active page */}
              <span className="text-[11px] font-bold text-[#C9A75D] tracking-[0.18em] uppercase">
                {activePageName}
              </span>
            </div>
          </div>
